...
- The format is encoded in the agent skill [skills/ht-industry-report/SKILL.md](skills/ht-industry-report/SKILL.md) (loaded as the system prompt) — edit it to change structure/tone without touching code. Citations are print-style (by name, never URLs); a server-side sanitizer strips any stray link artifacts.
...
- **No `OPENAI_API_KEY`?** Generation returns a deterministic stub so the whole app still demos.
