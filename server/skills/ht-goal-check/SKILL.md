...
2. **Company-specificity** — at least 2 sections explicitly connect evidence to the recipient's company by name; the report never presents an industry benchmark as if it were private company data, and never implies non-public knowledge. The executive summary contains at least one QUANTIFIED insight specific to the recipient's company or exact peer position — a report whose numbers are all generic industry statistics fails this rule.
...
6. **Exhibits** — 1–3 charts, each with 3–6 numeric data points and a `question` phrased as an analytical conclusion (not a topic label like "Denial rates" or "Retirement trends"); at most 2 sections carry `stats` (2–4 items each).
...
14. **Decision value** — where the report's evidence plausibly supports them: the cost of doing nothing is stated (quantified when derivable, directional otherwise) in the executive brief or agenda intro, and at least one body section frames a real decision as compared alternatives (tradeoff), not just a recommendation. Do not demand invented numbers — flag only when the evidence clearly allowed it and the report didn't use it.
